...
void GlobalVariable::initialize(ADC *adc)
{
  ...
  for (int i = 0; i < 8; i++)
  {
    this->dacCalibrationNeg[i] = 0;
  };
  for (int i = 0; i < 8; i++)
  {
    this->dacCalibrationPos[i] = 65535;
  };
  for (int i = 0; i < 4; i++)
  {
    this->adcCalibrationPos[i] = 0;
  };
  for (int i = 0; i < 4; i++)
  {
    this->adcCalibrationNeg[i] = 65535;
  };
  for (int i = 0; i < 4; i++)
  {
    this->adcCalibrationOffset[i] = 32767;
  };
...
uint16_t GlobalVariable::get_maximum_note_count(bool edo, uint16_t quantize_scale)
{
  if (edo)
  {
  }
  return 10 * quantize_scale;
}
uint16_t GlobalVariable::quantize_edo_dac_code_to_scale_degree(uint16_t dac_code, uint8_t scale_divisions)
{                                                                     //take dac_code input, and convert it to a scale degree based on current EDO scale division
  uint16_t single_division_dac_code = 65536 / (scale_divisions * 10); //10 octaves
  // Serial.printf("dac_code: %04d\tscale_divs: %04d single_div: %04d\n", single_division_dac_code, dac_code, scale_divisions);
  return dac_code / single_division_dac_code;
};
...
int16_t GlobalVariable::quantize_semitone_pitch(int16_t note, uint8_t quantize_key, uint16_t quantizeMode, bool direction)
{
  uint8_t count = 0;
...
uint16_t GlobalVariable::quantize_pitch_to_dac_code(uint16_t pitch, uint16_t quantize_scale)
{
...
uint16_t GlobalVariable::quantize_dac_code_to_pitch(uint16_t dac_code)
{
}

uint16_t GlobalVariable::quantize_dac_code(uint16_t dac_code)
{
}
