...
void DisplayModule::renderStringBox(uint8_t index, uint8_t highlight, int16_t x, int16_t y, int16_t w, int16_t h, bool border, uint8_t textSize, uint16_t color, uint16_t bgColor, bool forceHighlight)
{
  ...
  bool refresh = 0;
  if (previousColor[index] != color)
  {
    refresh = 1;
  };
  previousColor[index] = color;
  if (strcmp(displayElement[index], displayCache[index]) != 0)
  {
    refresh = 1;
  };
  if (previousStepMode != highlight && highlight == stepMode)
  {
    refresh = 1;
  };
  if (previousStepMode == highlight && highlight != stepMode)
  ...
  if (refresh)
  {
    if ((((highlight == stepMode)) && !globalObj->parameterSelect) || forceHighlight || ((globalObj->currentMenu == MENU_PATTERN_CHAIN) && (highlight == globalObj->chainSelectedPattern) && !globalObj->parameterSelect))
    {
      color1 = color;
      color2 = bgColor;
    }
...
void DisplayModule::stateDisplay_pitch(char *buf)
{
  ...
  if (globalObj->multiSelectSwitch)
  {
    ...
    if (globalObj->multi_pitch_switch)
    {
      switch(sequenceArray[globalObj->selectedChannel].quantizeScale){
        case 12:
          displayElement[1] = strdup(midiNotes[globalObj->multi_pitch % 128] );
          break;
        default:
      sprintf(buf, "%d-%d", globalObj->multi_pitch / sequenceArray[globalObj->selectedChannel].quantizeScale, globalObj->multi_pitch % sequenceArray[globalObj->selectedChannel].quantizeScale);
          displayElement[1] = strdup(buf);
      }
    }
    else
  ...
  else
  {
    ...
    switch(sequenceArray[globalObj->selectedChannel].quantizeScale){
      case 12:
        displayElement[1] = strdup(midiNotes[sequenceArray[globalObj->selectedChannel].getStepPitch(globalObj->selectedStep, 0) % 128] );
        break;
      default:
        sprintf(buf, "%d-%d", sequenceArray[globalObj->selectedChannel].getStepPitch(globalObj->selectedStep, 0) / sequenceArray[globalObj->selectedChannel].quantizeScale, sequenceArray[globalObj->selectedChannel].getStepPitch(globalObj->selectedStep, 0) % sequenceArray[globalObj->selectedChannel].quantizeScale);
        displayElement[1] = strdup(buf);
    }
...
void DisplayModule::scaleMenuDisplay()
{
  ...
  {
  case SEMITONE_SCALE_12:
    displayElement[8] = strdup("CHROMATIC");
    break;
  case MAJOR_SCALE_12:
    displayElement[8] = strdup("MAJOR");
    break;
  case MINOR_SCALE_12:
    displayElement[8] = strdup("MINOR");
    break;
  case MAJORMINOR_SCALE_12:
    displayElement[8] = strdup("MAJORMINOR");
    break;
  case BLUESMAJOR_SCALE_12:
    displayElement[8] = strdup("BLUESMAJOR");
    break;
  case BLUESMINOR_SCALE_12:
    displayElement[8] = strdup("BLUESMINOR");
    break;
  case PENT_MAJOR_SCALE_12:
    displayElement[8] = strdup("PENT_MAJOR");
    break;
  case PENT_MINOR_SCALE_12:
    displayElement[8] = strdup("PENT_MINOR");
    break;
  case FOLK_SCALE_12:
    displayElement[8] = strdup("FOLK");
    break;
  case AEOLIAN_SCALE_12:
    displayElement[8] = strdup("AEOLIAN");
    break;
  case DORIAN_SCALE_12:
    displayElement[8] = strdup("DORIAN");
    break;
  case MIXOLYDIAN_SCALE_12:
    displayElement[8] = strdup("MIXOLYDIAN");
    break;
  case PHRYGIAN_SCALE_12:
    displayElement[8] = strdup("PHRYGIAN");
    break;
  case LYDIAN_SCALE_12:
    displayElement[8] = strdup("LYDIAN");
    break;
  case LOCRIAN_SCALE_12:
    displayElement[8] = strdup("LOCRIAN");
    break;
  case GAMELAN_SCALE_12:
    displayElement[8] = strdup("GAMELAN");
    break;
  case JAPANESE_SCALE_12:
    displayElement[8] = strdup("JAPANESE");
    break;
  case GYPSY_SCALE_12:
    displayElement[8] = strdup("GYPSY");
    break;
  case ARABIAN_SCALE_12:
    displayElement[8] = strdup("ARABIAN");
    break;
  case FLAMENCO_SCALE_12:
    displayElement[8] = strdup("FLAMENCO");
    break;
  case WHOLETONE_SCALE_12:
    displayElement[8] = strdup("WHOLETONE");
    break;
  default:
    displayElement[8] = strdup("CUSTOM");
    break;
  }
...
void DisplayModule::patternSelectDisplay()
{
  ...
  {
  case PATTERN_CHANGE_IMMEDIATE:
    displayElement[22] = strdup("instant");
    break;
  case PATTERN_CHANGE_CHANNEL_0:
    displayElement[22] = strdup("ch1 reset");
    break;
  case PATTERN_CHANGE_CHANNEL_1:
    displayElement[22] = strdup("ch2 reset");
    break;
  case PATTERN_CHANGE_CHANNEL_2:
    displayElement[22] = strdup("ch3 reset");
    break;
  case PATTERN_CHANGE_CHANNEL_3:
    displayElement[22] = strdup("ch4 reset");
    break;
  }
...
void DisplayModule::patternChainHelpHandler()
{
  ...
  renderStringBox(i_ + 1, DISPLAY_LABEL, 0, ystart + i_ * yplus, 128, 8, false, REGULAR1X, background, contrastColor);
  i_++;
  renderStringBox(i_ + 1, DISPLAY_LABEL, 0, ystart + i_ * yplus, 128, 8, false, REGULAR1X, contrastColor, background);
  i_++;
  renderStringBox(i_ + 1, DISPLAY_LABEL, 0, ystart + i_ * yplus, 128, 8, false, REGULAR1X, background, contrastColor);
  i_++;
  renderStringBox(i_ + 1, DISPLAY_LABEL, 0, ystart + i_ * yplus, 128, 8, false, REGULAR1X, contrastColor, background);
    ...
    else
    {
      if (globalObj->chainPatternRepeatCount[patternChainIndex] > 0)
      {
        sprintf(buf, "%s%02d:%02d %02dx %s%s%s%s",
                ...
                (globalObj->chainModeMasterChannel[patternChainIndex] == 0) ? "k" : (globalObj->chainChannelMute[0][patternChainIndex] ? "m" : (globalObj->chainChannelSelect[0][patternChainIndex] ? "1" : "_")),
                (globalObj->chainModeMasterChannel[patternChainIndex] == 1) ? "k" : (globalObj->chainChannelMute[1][patternChainIndex] ? "m" : (globalObj->chainChannelSelect[1][patternChainIndex] ? "2" : "_")),
                (globalObj->chainModeMasterChannel[patternChainIndex] == 2) ? "k" : (globalObj->chainChannelMute[2][patternChainIndex] ? "m" : (globalObj->chainChannelSelect[2][patternChainIndex] ? "3" : "_")),
                (globalObj->chainModeMasterChannel[patternChainIndex] == 3) ? "k" : (globalObj->chainChannelMute[3][patternChainIndex] ? "m" : (globalObj->chainChannelSelect[3][patternChainIndex] ? "4" : "_")));
      }
