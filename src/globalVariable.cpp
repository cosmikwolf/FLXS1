...
uint16_t GlobalVariable::quantize_edo_pitch(uint16_t note, uint16_t quantizeKey, uint32_t quantize_mode, uint8_t scale_divisions, bool direction)
{
  ...
  if (quantize_mode == 0)
  {
    quantize_mode = 0xffff;
  }
  scale_divisions -= 1; //minus one for the octave. 11 notes in a 12-tet scale, etc
  ...
  //Serial.println("\tquantize_mode: " + String(scaleExpanded, BIN) + "\tquantizedNote: " + String(note) + "\tcount: " + String(count));
  return note;
};
